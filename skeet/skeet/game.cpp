...
Bird* Game :: createBird()
{
    ...
    //Setting a randome point for the randome bird
    point.setX(-200);
    point.setY(random(-200, 200));

    //Random selection for which bird will be created next
   int selection = random(0, 3);
    
    if(selection == 0)
    {
        newBird = new StandardBird(point);
    }
    else if (selection == 1)
    {
        newBird = new ToughBird(point);
    }
    else if(selection == 2)
    {
        newBird = new SacredBird(point);
    }
